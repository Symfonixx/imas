...
<x-admin-layout>
    <form method="POST" action="{{ route('admin.corporate_testimonials.update', $testimonial->id) }}"
        ...
        <div class="row gx-5 gx-xl-10">
                    ...
                    <div class="card-body pt-0">
                        ...
                        <x-admin.form-group label="Client" name="client" required>
                            <input type="text" name="client" class="form-control form-control-solid"
                                   value="{{ old('client', $testimonial->client) }}" placeholder="{{ __('Client') }}"/>
                        </x-admin.form-group>
                        ...
                        <x-admin.form-group label="Position" name="position" translatable>
                            <input type="text" name="position" class="form-control form-control-solid"
                                   value="{{ old('position', $testimonial->position) }}" placeholder="{{ __('Position') }}"/>
                        </x-admin.form-group>
                        ...
                        <x-admin.form-group label="Quote" name="quote" required translatable>
                            <textarea name="quote" rows="4" class="form-control form-control-solid"
                                      placeholder="{{ __('Quote') }}">{{ old('quote', $testimonial->quote) }}</textarea>
                        </x-admin.form-group>
                        ...
                        <x-admin.form-group label="Link" name="link">
                            <input type="url" name="link" class="form-control form-control-solid"
                                   value="{{ old('link', $testimonial->link) }}" placeholder="https://"/>
                        </x-admin.form-group>
                    </div>
                </div>
            </div>
            ...
            <div class="col-xxl-4 col-xl-4">
                @include('cms::admin.partials._status_aside', [
                    'isActive' => old('publish', $testimonial->status) === 'Published',
                    'isFeatured' => false,
