...
@section('toolbar')
    ...
    <div class="d-flex align-items-center gap-2 gap-lg-3">
        <a class="btn btn-sm fw-bold btn-primary" href="{{ route('admin.corporate_testimonials.create') }}">
...
<x-admin-layout>
    <x-admin.table :model="$model" :search="__('Search In Testimonials')" :form-url="route('admin.corporate_testimonials.deleteMulti')">
        <thead>
            ...
            <tr>
                ...
                <td>
                    <div class="symbol symbol-60px">
                        <img src="{{ $row->avatar_link }}" alt="" class="rounded object-fit-cover w-100 h-100"/>
                    </div>
                </td>
                <td><span class="fw-bolder fs-6">{{ $row->name }}</span></td>
                <td>{{ $row->client }}</td>
                <td>{{ $row->rank }}</td>
                <td>
                    <span
                        class="badge badge-light-{{ $row->status == 'Published' ? 'success' : 'warning' }} fs-7 fw-bold">{{ __($row->status) }}</span>
                </td>
                <td>{{ $row->created_at->diffForHumans() }}</td>
                <td class="text-end">
                    <a href="{{ route('admin.corporate_testimonials.edit', $row->id) }}"
