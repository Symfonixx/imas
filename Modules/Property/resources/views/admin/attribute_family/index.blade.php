...
@section('toolbar')
    @php
        $breadcrumbItems = [
            ...
            ['label' => __('Attribute families')],
        ];
    @endphp
    <x-admin.breadcrumb :pageTitle="__('Attribute families')" :breadcrumbItems="$breadcrumbItems"/>
    <div class="d-flex align-items-center gap-2 gap-lg-3">
        <a class="btn btn-sm fw-bold btn-primary" href="{{ route('admin.attribute_families.create') }}">
            {{ __('Add attribute family') }}
        </a>
    </div>
@endsection

<x-admin-layout>
    <x-admin.table :model="$model" :search="__('Search in attribute families')" :form-url="route('admin.attribute_families.deleteMulti')">
        <thead>
        ...
        @foreach($model as $row)
            <tr>
                ...
                <td class="text-end">
                    <a href="{{ route('admin.attribute_families.edit', $row) }}"
