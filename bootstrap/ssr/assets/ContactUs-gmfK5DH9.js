...
import { i as normalizeTurkishPhoneDigits, n as resolveWhatsAppContactHref, o as localizedRoute, r as formatTurkishPhone, t as _sfc_main$3 } from "./App-BhEC2Bhd.js";
...
import { t as ContactForm_default } from "./ContactForm-CF4Tl_3a.js";
	...
	setup(__props) {
		...
		return (_ctx, _push, _parent, _attrs) => {
			if (items.value.length) {
				...
				}, _attrs))}><h3 id="imas-contact-faq-title" class="imas-contact-page__heading imas-contact-faq__title">${ssrInterpolate(title.value)}</h3>`);
				if (subtitle.value) _push(`<p class="imas-contact-faq__subtitle text-dim text-card-excerpt">${ssrInterpolate(subtitle.value)}</p>`);
				else _push(`<!---->`);
				_push(`<ul class="accordion accordion-1 one-open imas-contact-faq__list"><!--[-->`);
				ssrRenderList(items.value, (item, index) => {
					_push(`<li class="${ssrRenderClass({ active: openIndex.value === index })}"><button type="button" class="title imas-contact-faq__trigger"${ssrRenderAttr("aria-expanded", openIndex.value === index)}><span>${ssrInterpolate(item.question)}</span></button><div class="content imas-contact-faq__content"><p>${ssrInterpolate(item.answer)}</p></div></li>`);
				});
				_push(`<!--]--></ul></section>`);
			} else _push(`<!---->`);
		};
			...
			_push(ssrRenderComponent(_sfc_main$3, null, {
				default: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) {
						...
						_push(`</div></div></div><div class="col-12"${_scopeId}>`);
						...
						"contact-store-url": __props.contactStoreUrl,
						"source-page": trans("contact_us.title")
					}, null, 8, ["contact-store-url", "source-page"])])]), createVNode("div", { class: "col-lg-4 col-md-12" }, [createVNode("div", { class: "imas-contact-page__panel imas-contact-page__panel--details" }, [createVNode(ContactDetails_default)])])]), createVNode("div", { class: "col-12" }, [createVNode(_sfc_main$1)])])])], 512)];
				}),
				_: 1
			}, _parent));
			_push(`<!--]-->`);
