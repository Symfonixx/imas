import { n as resolveYoutubeHeroBackgroundSrc } from "./videoEmbed-6-mdNPXm.js";
import { Link, usePage } from "@inertiajs/vue3";
import { Fragment, computed, createBlock, createTextVNode, createVNode, mergeProps, onBeforeUnmount, onMounted, openBlock, ref, renderList, resolveDynamicComponent, toDisplayString, unref, useSSRContext, watch, withCtx } from "vue";
	...
	setup(__props) {
		...
		*/
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			_push(`</div><div class="imas-inner-page-heading-hero__inner">`);
			ssrRenderVNode(_push, createVNode(resolveDynamicComponent(__props.titleTag), { class: ["imas-inner-page-heading-hero__title", { "imas-inner-page-heading-hero__title--connected": titleUsesConnectedTitle.value }] }, {
				default: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) if (titleUsesConnectedTitle.value) _push(`<span class="imas-inner-page-heading-hero__title-text"${_scopeId}>${ssrInterpolate(displayTitle.value)}</span>`);
					else {
						_push(`<!--[-->`);
						ssrRenderList(titleLetters.value, (ch, i) => {
							_push(`<span class="imas-inner-page-heading-hero__letter" style="${ssrRenderStyle({ animationDelay: `${120 + i * 90}ms` })}"${_scopeId}>${ssrInterpolate(ch === " " ? "\xA0" : ch)}</span>`);
						});
						_push(`<!--]-->`);
					}
					else return [titleUsesConnectedTitle.value ? (openBlock(), createBlock("span", {
						key: 0,
						class: "imas-inner-page-heading-hero__title-text"
					}, toDisplayString(displayTitle.value), 1)) : (openBlock(true), createBlock(Fragment, { key: 1 }, renderList(titleLetters.value, (ch, i) => {
						return openBlock(), createBlock("span", {
							key: i,
							class: "imas-inner-page-heading-hero__letter",
							style: { animationDelay: `${120 + i * 90}ms` }
						}, toDisplayString(ch === " " ? "\xA0" : ch), 5);
					}), 128))];
				}),
				_: 1
			}), _parent);
