...
import { o as localizedRoute, t as _sfc_main$3 } from "./App-6l5p54Dj.js";
...
var _sfc_main$1 = {
	...
	setup(__props) {
		...
		const hasActiveFilters = computed(() => {
			...
			const locationIds = Array.isArray(f.location_id) ? f.location_id.filter((id) => id != null && id !== "") : f.location_id != null && f.location_id !== "" ? [f.location_id] : [];
			const unitIds = Array.isArray(f.project_unit_type_id) ? f.project_unit_type_id.filter((id) => id != null && id !== "") : f.project_unit_type_id != null && f.project_unit_type_id !== "" ? [f.project_unit_type_id] : [];
			if (locationIds.length > 0) return true;
			if (f.property_type_id != null && f.property_type_id !== "") return true;
			if (unitIds.length > 0) return true;
			if (typeof f.q === "string" && f.q.trim() !== "") return true;
			if (f.min_price != null || f.max_price != null || f.min_area != null || f.max_area != null) return true;
			return searchCityIds.value.length > 0 || searchLocationIds.value.length > 0 || searchPropertyTypeId.value !== "" || searchUnitTypeId.value !== "" || rangesDirty.value;
		});
		function trans(key) {
			return page.props.translations[key] || key;
		}
...
var _sfc_main = {
	...
	setup(__props) {
		...
		const listingFiltersAreActive = computed(() => {
			...
			if (unitIds.length > 0) return true;
			if (typeof f.q === "string" && f.q.trim() !== "") return true;
			return f.min_price != null || f.max_price != null || f.min_area != null || f.max_area != null;
		});
