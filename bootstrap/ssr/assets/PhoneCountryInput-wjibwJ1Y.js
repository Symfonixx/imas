import { ref, computed, watch, nextTick, onMounted, onBeforeUnmount, mergeProps, useSSRContext } from "vue";
...
const _sfc_main = {
  ...
  setup(__props, { emit: __emit }) {
    ...
    watch(countryDropdownOpen, (open) => {
      ...
      nextTick(() => {
        var _a, _b;
        (_b = (_a = countrySearchInput.value) == null ? void 0 : _a.focus) == null ? void 0 : _b.call(_a);
      });
    });
    function pickDefaultCountry() {
      ...
      if (countryId.value != null && list.some((c) => c.id === countryId.value)) {
        return;
      }
    ...
    function parseMobileWithCountries(fullMobile, list) {
      ...
      const sorted = [...list].sort(
        (a, b) => digitsOnly(b.phone_code).length - digitsOnly(a.phone_code).length
      );
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if ((_a = selectedCountry.value) == null ? void 0 : _a.flag) {
        _push(`<img class="imas-auth-country-flag-img"${ssrRenderAttr("src", selectedCountry.value.flag)} alt="" width="22" height="16" decoding="async" loading="lazy" data-v-3f1b4306>`);
      } else {
        _push(`<!---->`);
      }
      _push(`<span class="imas-auth-country-code-label" aria-hidden="true" data-v-3f1b4306>+${ssrInterpolate(displayCallingCode((_b = selectedCountry.value) == null ? void 0 : _b.phone_code))}</span></button><div style="${ssrRenderStyle(countryDropdownOpen.value ? null : { display: "none" })}" class="imas-auth-country-dropdown-panel" data-v-3f1b4306><div class="imas-auth-country-dropdown-search-wrap text-start" data-v-3f1b4306><input${ssrRenderAttr("value", countrySearchQuery.value)} type="search" enterkeyhint="search" autocomplete="off" autocorrect="off" spellcheck="false" class="imas-auth-country-dropdown-search"${ssrRenderAttr("placeholder", trans("Search"))}${ssrRenderAttr("aria-label", trans("Search"))} data-v-3f1b4306></div><ul class="imas-auth-country-dropdown-scroll" role="listbox" tabindex="-1" data-v-3f1b4306>`);
      if (countriesWithPhoneFiltered.value.length === 0) {
        _push(`<li class="imas-auth-country-option imas-auth-country-option--empty" aria-live="polite" data-v-3f1b4306>${ssrInterpolate(trans("auth_modal.country_code_search_empty"))}</li>`);
      } else {
        _push(`<!---->`);
      }
      _push(`<!--[-->`);
      ssrRenderList(countriesWithPhoneFiltered.value, (c) => {
        _push(`<li role="option" class="${ssrRenderClass([{
          "imas-auth-country-option--selected": c.id === countryId.value
        }, "imas-auth-country-option"])}"${ssrRenderAttr("aria-selected", c.id === countryId.value)} data-v-3f1b4306>`);
        if (c.flag) {
          _push(`<img class="imas-auth-country-flag-img imas-auth-country-flag-img--option"${ssrRenderAttr("src", c.flag)} alt="" width="22" height="16" decoding="async" loading="lazy" data-v-3f1b4306>`);
        } else {
          _push(`<!---->`);
        }
...
const PhoneCountryInput = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-3f1b4306"]]);
export {
  PhoneCountryInput as P
};
