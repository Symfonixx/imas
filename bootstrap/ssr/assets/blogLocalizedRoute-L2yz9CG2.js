import { o as localizedRoute } from "./App-DkOZMeWI.js";
	...
	setup(__props) {
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			if (__props.showSearch && __props.searchAction) {
				...
				if (__props.filters.category) _push(`<input type="hidden" name="category"${ssrRenderAttr("value", __props.filters.category)}>`);
				else _push(`<!---->`);
			...
			if (__props.showCategories) {
				...
				_push(ssrRenderComponent(unref(Link), {
					...
					class: ["imas-blog-v2-sidebar__cat-link", { "is-active": !__props.filters.category }]
				}, {
				...
				ssrRenderList(__props.categories, (c) => {
					_push(`<li>`);
					_push(ssrRenderComponent(unref(Link), {
						href: __props.categoryUrl(c.slug),
						class: ["imas-blog-v2-sidebar__cat-link", { "is-active": __props.filters.category != null && __props.filters.category === c.slug }]
					}, {
						default: withCtx((_, _push, _parent, _scopeId) => {
			...
			if (__props.showRecentPosts) {
				...
				ssrRenderList(__props.recentBlogs, (r) => {
					_push(`<a${ssrRenderAttr("href", r.url)} class="imas-blog-v2-sidebar__recent-item"><img${ssrRenderAttr("src", r.image)}${ssrRenderAttr("alt", r.title)} loading="lazy"><div class="imas-blog-v2-sidebar__recent-body"><div class="imas-blog-v2-sidebar__recent-title">${ssrInterpolate(r.title)}</div>`);
					if (r.date) _push(`<div class="imas-blog-v2-sidebar__recent-date text-dim">${ssrInterpolate(r.date)}</div>`);
					else _push(`<!---->`);
					_push(`</div></a>`);
				});
				_push(`<!--]--></div></div>`);
