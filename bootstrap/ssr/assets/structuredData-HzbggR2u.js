import { createTextVNode, createVNode, mergeProps, resolveDynamicComponent, toDisplayString, useSSRContext, withCtx } from "vue";
import { ssrInterpolate, ssrRenderVNode } from "vue/server-renderer";
//#region resources/js/components/Seo/JsonLdScript.vue
var _sfc_main = {
	__name: "JsonLdScript",
	...
	setup(__props) {
		return (_ctx, _push, _parent, _attrs) => {
			if (__props.content) ssrRenderVNode(_push, createVNode(resolveDynamicComponent("script"), mergeProps({
				"head-key": __props.headKey,
				type: "application/ld+json"
			}, _attrs), {
				default: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) _push(`${ssrInterpolate(__props.content)}`);
					else return [createTextVNode(toDisplayString(__props.content), 1)];
				}),
				_: 1
			}), _parent);
			else _push(`<!---->`);
...
//#endregion
export { buildWebsiteSchema as a, stripHtml as c, buildRealEstateListingSchema as i, _sfc_main as l, buildBreadcrumbSchema as n, collectSocialUrls as o, buildOrganizationSchema as r, filterSchemaImages as s, buildArticleSchema as t };
