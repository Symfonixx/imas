...
//#region resources/js/composables/useBoundedSticky.js
function getStickOffset(extraGap = 16) {
	const header = document.getElementById("header");
	if (!header) return extraGap;
	const rect = header.getBoundingClientRect();
	if (header.classList.contains("imas-scroll-pinned--in") || rect.top <= 0) return Math.round(rect.height + extraGap);
...
*/
function useBoundedSticky({ boundaryRef: _boundaryRef, columnRef: _columnRef, targetRef, minWidth = 992 }) {
	let raf = 0;
	...
	onMounted(() => {
		...
		if (typeof ResizeObserver !== "undefined") {
			...
			if (targetRef.value) resizeObserver.observe(targetRef.value);
		}
		scheduleUpdate();
