...
function useDocumentSeo(options = {}) {
	...
	*/
	function siteOrigin() {
		...
		if (typeof ziggyUrl === "string" && ziggyUrl.trim() !== "") return ziggyUrl.trim().replace(/\/$/, "");
	...
	*/
	...
	const description = computed(() => {
		...
		if (explicit) return explicit;
		const fromKeys = pickSeoString(...options.descriptionKeys ?? ["site_meta_description", "website_desc"]);
	...
	const ogImage = computed(() => {
		...
		if (explicit && !isDefaultPlaceholder(explicit)) return toAbsoluteUrl(explicit);
	...
	const canonical = computed(() => toAbsoluteUrl(resolveOption(options.canonical)));
	return {
		siteName,
		globals,
		seo: seoMap,
		...
		robots: computed(() => asTrimmedString(resolveOption(options.robots))),
