...
function loadJqueryUi(themeUrl) {
	...
	jqueryUiPromise = import("./loadThemeAsset-ClrtYxqQ.js").then(({ loadScript, loadStylesheet }) => loadStylesheet(`${base}/css/jquery-ui.css`).then(() => loadScript(`${base}/js/jquery-ui.js`))).catch((err) => {
...
*/
function setHeroRangeSliderValues({ areaSelector = "#imas-hero-area-range", priceSelector = "#imas-hero-price-range", areaValues, priceValues, areaUnit = "m²", priceUnit = "$" } = {}) {
	const $ = window.jQuery;
	if (!$?.fn?.slider) return;
	if (Array.isArray(areaValues) && areaValues.length === 2) {
		...
		if ($area.hasClass("ui-slider")) {
			...
			$area.children(".first-slider-value").val(`${areaValues[0]} ${areaUnit}`);
			$area.children(".second-slider-value").val(`${areaValues[1]} ${areaUnit}`);
		}
	}
	if (Array.isArray(priceValues) && priceValues.length === 2) {
		const $price = $(priceSelector);
		if ($price.hasClass("ui-slider")) {
			const format = (n) => priceUnit + Number(n).toString().replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,");
...
//#endregion
export { destroyHeroRangeSliders as a, setHeroRangeSliderValues as c, LocationAreaPicker_default as i, splitLocationIds as n, initHeroRangeSliders as o, LocationCityPicker_default as r, loadJqueryUi as s, useLocationSearchFilters as t };
