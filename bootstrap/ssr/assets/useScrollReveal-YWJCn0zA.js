...
import { u as useGsap } from "./App-BMYoBaMl.js";
const PRESETS = {
  default: {
    ease: "power2.out",
    y: 56,
...
function useScrollReveal(sectionRef, options = {}) {
  ...
  const {
    ...
    variant = "section",
    start = "top 88%",
    sectionSelector = "[data-imas-reveal]"
  } = options;
  ...
  function buildAnimation(root) {
    ...
    if (variant === "panel") {
      const panel = root.querySelector(
        ".imas-tc-overview__panel, .imas-about-overview__panel"
      ) ?? root;
      gsap.from(panel, {
        opacity: 0,
        y: 64,
    ...
    if (variant === "cards") {
      ...
      if (title) {
        tl.from(
          title,
          { opacity: 0, y: 40, duration: T.titleDuration },
          0
        );
      }
      if (items.length) {
      ...
      if (footer) {
        tl.from(
          footer,
          { opacity: 0, y: 32, duration: T.footerDuration },
          items.length ? "-=0.25" : title ? "-=0.15" : 0
        );
      }
      if (!title && !items.length && !footer) {
        gsap.from(root, {
          opacity: 0,
          y,
          duration,
    ...
    if (variant === "propertyListings") {
      ...
      if (toolbar) {
        tl.from(
          toolbar,
          { opacity: 0, y: 36, duration: T.itemDuration },
          heading ? "-=0.35" : 0
        );
      }
      if (cards.length) {
      ...
      if (!heading && !toolbar && !cards.length && !sidebarBlocks.length && !pagination) {
        gsap.from(root, {
          opacity: 0,
          y,
          duration,
    ...
    if (variant === "blogShowArticle") {
      ...
      const header = root.querySelector(
        ".imas-blog-show-article-text__header"
      );
    ...
    if (variant === "carousel") {
      ...
      const title = root.querySelector(
        ".sec-title, .imas-custom-heading"
      );
      const track = root.querySelector(
        ".owl-carousel, .imas-popular-rail"
      );
      if (title) {
        tl.from(
          title,
      ...
      if (track) {
        tl.from(
          track,
          { opacity: 0, y: 52, duration: T.duration },
          title ? "-=0.35" : 0
        );
      } else if (!title) {
        gsap.from(root, { opacity: 0, y, duration, scrollTrigger });
      }
      return;
    }
    gsap.from(root, {
  ...
  function setup() {
    ...
    if (when != null) {
      const active = typeof when === "function" ? when() : when.value;
      if (!active) {
        return;
      }
    }
